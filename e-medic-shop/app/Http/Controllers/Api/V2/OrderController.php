...
class OrderController extends Controller
{
    public function store(Request $request, $set_paid = false)
    {
        ...
        foreach ($cartItems as $cartItem) {
            $item_sum = 0;
            $item_sum += ($cartItem->price + $cartItem->tax + $cartItem->profit_margin) * $cartItem->quantity;
            $item_sum += $cartItem->shipping_cost - $cartItem->discount;
            $sum += $item_sum;   //// 'grand_total' => $request->g

            $profit_margin +=  $cartItem->profit_margin;
        }
        ...
        foreach ($cartItems as $cartItem) {
            ...
            OrderDetail::create([
                ...
                'price' => ($cartItem->price + $cartItem->profit_margin) * $cartItem->quantity,
                'profit_margin' => $cartItem->profit_margin,
